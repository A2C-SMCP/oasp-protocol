...
### 元素标识符不透明性 {#element-id-opacity}

幻灯片元素标识符——`SlideElement.id` 以及各 chart 事件的 `elementId`——是**服务端分配的不透明字符串**。消费方**不得**解析、推断或依赖其内部结构与格式，只能将其作为整体令牌原样回传。

这是规范层约束，独立于实现：实现可自由更换内部 id 生成方式（例如整页 round-trip 后改用稳定 UUID 并写入 OOXML `cNvPr/@name`），只要同一元素在其生命周期内返回稳定、可回传的标识符即可——协议不绑定任何具体格式。
